...
int main(int argc, char **argv) {
  ...
  xp = &x;
  cp = &c;
  ip = 0;  // ◄── "Proves" MACRO is resolved at compile time
  printf("  func(xp)  = %d\n",MACRO(xp));
  printf("  func(&xp) = %d\n",MACRO(&xp));
  printf("  func(cp)  = %d\n",MACRO(cp));
  printf("  func(&cp) = %d\n",MACRO(&cp));
  printf("  func(ip)  = %d\n",MACRO(ip));
  printf("  func(&ip) = %d\n",MACRO(&ip));
  printf("  func(vp)  = %d\n",MACRO(vp));  // ◄┬─ Oddly, shows gcc 4.8.1 sizeof(void) == 1
  printf("  func(&vp) = %d\n",MACRO(&vp)); // ◄┘  I expected this to produce compile error because
                                           //     of '/' in MACRO but you learn something new every day!!!
