...
# File:    BNF_Enum.py
#
# Purpose: Enum and other related datatype classes
#
# Usage:   python BNF_Enum.py
#
#===============================================================================
...
class Enum(object):
    ...
    for memb in members:
      ...
      if numParts < 1:
        ...
        try:
          ...
          raise AttributeError("Enum expected integer value in member '%s'"%name)
        name = name.strip()
        cur = value
        if not name:
          continue
  ...
  def __getitem__(self, index):
    ...
    if typ in [int,long]:
      ...
      raise IndexError("Enum can only be indexed by a string or integer, got %s"%repr(typ))
